...
import type { Species, BaseStats, EVYield, LevelMove, Evolution, GenderRatio } from '../schema/species.js';
import type { Provenance, ValidationError } from '../schema/common.js';

function parseBaseStats(raw: string): BaseStats {
  const [hp, atk, def, spe, spa, spd] = raw.split(',').map(Number);
  return { hp, atk, def, spe, spa, spd };
}

function parseEvYield(raw: string): EVYield[] {
  const parts = raw.split(',');
  const result: EVYield[] = [];
...
function parseLevelMoves(raw: string): LevelMove[] {
  const parts = raw.split(',');
  const result: LevelMove[] = [];
  for (let i = 0; i < parts.length - 1; i += 2) {
...
function parseEvolutions(raw: string): Evolution[] {
  const parts = raw.split(',');
...
export function parseSpeciesBlock(
  id: string,
  block: PbsBlock,
  ...
  const types = [rawTypes[0], rawTypes[1]] as [string, string?];
  ...
  return {
    ...
    genderRatio: (block['GenderRatio'] ?? 'Female50Percent') as GenderRatio,
