...
function runRomNarcImport(romPath, narcPath, extractFiles, extractLimit) {
...
function runPbsImport(validateOnly) {
    const parseErrors = [];
    function parseCatalog(pbsMap, parser, provenance) {
    ...
    const baseProv = { source: 'essentials-21.1', file: 'PBS/pokemon.txt' };
    const baseMoveProv = { source: 'essentials-21.1', file: 'PBS/moves.txt' };
    const baseAbilityProv = { source: 'essentials-21.1', file: 'PBS/abilities.txt' };
    ...
    const gen9Prov = { source: 'gen9-pack-3.3.4', file: 'PBS/pokemon_base_Gen_9_Pack.txt' };
    const gen9MoveProv = { source: 'gen9-pack-3.3.4', file: 'PBS/moves_Gen_9_Pack.txt' };
    const gen9AbilityProv = { source: 'gen9-pack-3.3.4', file: 'PBS/abilities_Gen_9_Pack.txt' };
    ...
    console.log('\nMerging datasets...');
    const allConflicts = [];
    const speciesFields = ['name', 'types', 'baseStats', 'genderRatio', 'growthRate', 'catchRate'];
    const moveFields = ['name', 'type', 'category', 'power', 'accuracy', 'pp', 'target', 'functionCode'];
    const abilityFields = ['name', 'description'];
    const { merged: species, conflicts: speciesConflicts } = mergeWithConflicts(baseSpecies, gen9Species, 'gen9-pack-3.3.4', 'essentials-21.1', speciesFields);
    const { merged: moves, conflicts: moveConflicts } = mergeWithConflicts(baseMoves, gen9Moves, 'gen9-pack-3.3.4', 'essentials-21.1', moveFields);
    const { merged: abilities, conflicts: abilityConflicts } = mergeWithConflicts(baseAbilities, gen9Abilities, 'gen9-pack-3.3.4', 'essentials-21.1', abilityFields);
