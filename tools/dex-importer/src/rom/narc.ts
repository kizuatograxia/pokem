...
export interface NarcChunkInfo {
  id: string;
  offset: number;
  size: number;
}

export interface NarcSubfileInfo {
  index: number;
  start: number;
  end: number;
  size: number;
...
export interface NarcHistogramEntry {
  magic: string;
  count: number;
}

export interface RomNarcInspectionResult {
  romPath: string;
  romFileName: string;
  romSha256: string;
  narcPath: string;
  narcSize: number;
  fileCount: number;
  chunks: NarcChunkInfo[];
  files: NarcSubfileInfo[];
  magicHistogram: NarcHistogramEntry[];
}
...
function readRomSlice(romPath: string, offset: number, size: number) {
...
function getAsciiSignature(buffer: Buffer, offset: number) {
...
function readLzSize(buffer: Buffer) {
...
function decompressLz10(buffer: Buffer) {
...
function decompressLz11(buffer: Buffer) {
  const { size, headerSize } = readLzSize(buffer);
  const output = Buffer.alloc(size);
  let input = headerSize;
  let out = 0;
...
function detectInnerMagic(buffer: Buffer) {
  const ascii = getAsciiSignature(buffer, 0);
  return ascii || 'BIN';
}

function detectSubfileMagic(buffer: Buffer) {
  if (buffer.length === 0) {
    return 'EMPTY';
  }
...
export function inspectRomNarc(romPath: string, narcPath: string): RomNarcInspectionResult {
...
export function writeRomNarcInspection(
  outputDir: string,
  inspection: RomNarcInspectionResult,
  options: { extractFiles: boolean; extractLimit: number | null }
) {
