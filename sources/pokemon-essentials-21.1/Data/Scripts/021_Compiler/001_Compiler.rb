...
module Compiler
  ...
  def main
    force_compile_file = "Data/force_compile"
    force_compile = FileTest.exist?(force_compile_file)
    return if !$DEBUG && !force_compile
    begin
      mustCompile = false
      ...
      Input.update if $DEBUG
      mustCompile = true if $DEBUG && Input.press?(Input::CTRL)
      mustCompile = true if force_compile
      ...
      if force_compile && !mustCompile && FileTest.exist?(force_compile_file)
        File.delete(force_compile_file) rescue nil
      elsif force_compile && mustCompile && FileTest.exist?(force_compile_file)
        File.delete(force_compile_file) rescue nil
      end
    rescue Exception
      e = $!
