...
import type { BattleViewState, PlayerCommand, MoveCommand, SwitchCommand, EffectStyle } from '../view-model/types.js';

function encodeMoveChoice(cmd: MoveCommand): string {
  ...
  if (cmd.mega) modifiers.push('mega');
  if (cmd.tera) modifiers.push('terastallize');
  if (cmd.dynamax) modifiers.push('dynamax');
  return modifiers.length ? `${base} ${modifiers.join(' ')}` : base;
}

export interface SdTeamSpec {
  name: string;
  team: string; // Packed team format (Showdown import/export)
}
...
export class SdAdapter {
  ...
  private _battle: unknown = null; // @pkmn/sim Battle instance — typed after install
  ...
  async start(p1: SdTeamSpec, p2: SdTeamSpec): Promise<void> {
    try {
      ...
      const battle = this._battle as {
        choose: (side: string, input: string) => void;
        sendUpdates: () => void;
        started: boolean;
      };

      // Add players using the Battle API
      (this._battle as any).setPlayer('p1', { name: p1.name, team: p1.team });
      (this._battle as any).setPlayer('p2', { name: p2.name, team: p2.team });
      battle.sendUpdates();
  ...
  command(cmd: PlayerCommand): void {
    ...
    switch (cmd.kind) {
      ...
      case 'doubles-choice':
        ...
        chooseStr = cmd.choices
          ...
          .map(c => c.choice.kind === 'move' ? encodeMoveChoice(c.choice) : `switch ${(c.choice as SwitchCommand).partyIndex}`)
          .join(', ');
        break;
      default:
        return;
    ...
    const battle = this._battle as { choose: (side: string, input: string) => void; sendUpdates: () => void };
