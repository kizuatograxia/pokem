import type { ConflictRecord, SourceLayer } from './schema/common.js';
 ...
 */
export function mergeWithConflicts<T extends { id: string }>(
  base: Map<string, T>,
  expansion: Map<string, T>,
  expansionLayer: SourceLayer,
  baseLayer: SourceLayer,
  compareFields: (keyof T)[]
): { merged: Map<string, T>; conflicts: ConflictRecord[] } {
  ...
  for (const [id, expEntry] of expansion) {
    ...
    for (const field of compareFields) {
      ...
      if (SILENT_OVERRIDE_FIELDS.has(fieldName)) continue;
