...
function readUtf16(buffer, offset, byteLength) {
    return buffer
...
function readSection(buffer, offset) {
...
function readExecutableSection(buffer, offset) {
...
function parseBanner(buffer, offset) {
...
function joinNitroPath(parent, name) {
    return parent.length > 0 ? `${parent}/${name}` : name;
}
function parseFilesystem(buffer, header, warnings) {
    ...
    if (fileNameTable.offset >= buffer.length ||
    ...
    const walkDirectory = (dirIndex, parentPath) => {
...
export function inspectRom(romPath) {
    ...
    if (header.romSizeDeclared > buffer.length) {
        warnings.push(`Declared ROM size (${header.romSizeDeclared}) is larger than the actual file size (${buffer.length}).`);
    }
