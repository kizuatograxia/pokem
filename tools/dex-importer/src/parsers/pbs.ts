...
export type PbsBlock = Record<string, string>;
export type PbsFile = Map<string, PbsBlock>;

/**
 * Parses a PBS file into a map of id -> key/value block.
 * Handles BOM, comment lines, section separators, and multi-value fields.
 */
export function parsePbsFile(filePath: string): PbsFile {
  const raw = readFileSync(filePath, 'utf-8').replace(/^\uFEFF/, ''); // strip BOM
  const result: PbsFile = new Map();

  let currentId: string | null = null;
  let currentBlock: PbsBlock = {};
  ...
  for (const rawLine of raw.split(/\r?\n/)) {
    ...
    if (eqIdx === -1 || currentId === null) continue;
