import type { SdEvent, SdPokemonIdent, SdRequest, SdSide } from './types.js';

// ─── Ident parsing ───────────────────────────────────────────────────────────
// Showdown ident format: "p1a: Charizard" or just "p1: Charizard"
// Details format:        "Charizard, L50, M" or "Charizard"

function parseSide(raw: string): SdSide {
  return raw.startsWith('p2') ? 'p2' : 'p1';
}

function parseIdent(identStr: string, detailsStr = ''): SdPokemonIdent {
  ...
  const side: SdSide = locPart.startsWith('p2') ? 'p2' : 'p1';
  ...
  for (const part of parts.slice(1)) {
    if (part.startsWith('L')) level = parseInt(part.slice(1), 10);
...
export function parseHpStatus(raw: string): { hpCurrent: number; hpMax: number; percent: number; status: string | null; fainted: boolean } {
  const fainted = raw === '0 fnt' || raw.endsWith(' fnt');
  if (fainted) return { hpCurrent: 0, hpMax: 100, percent: 0, status: null, fainted: true };
...
export function parseShowdownLine(line: string): SdEvent | null {
  ...
  switch (cmd) {
    case 'player':
      return { type: 'player', side: parts[1] as SdSide, name: parts[2] ?? '', avatar: parts[3] ?? '' };

    case 'teamsize':
      return { type: 'teamsize', side: parts[1] as SdSide, size: parseInt(parts[2], 10) };
    ...
    case 'request': {
      try {
        const json = JSON.parse(parts[1] ?? '{}') as SdRequest;
...
function extractFrom(parts: string[]): string | undefined {
  for (const p of parts) {
    if (p.startsWith('[from]')) return p.slice(7).trim();
  }
  return undefined;
}

export function parseShowdownChunk(chunk: string): SdEvent[] {
  return chunk
    .split('\n')
    .map(line => parseShowdownLine(line.trim()))
    .filter((e): e is SdEvent => e !== null);
}
