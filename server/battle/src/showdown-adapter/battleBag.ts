import type { BattleBagPocketView } from "../view-model/types.js";

export type BattleBagUseType = "pokemon" | "battler" | "no-target" | "ball" | "field";

export interface BattleBagItemState {
  id: string;
  name: string;
  description: string;
  quantity: number | null;
  useType: BattleBagUseType;
}

export interface BattleBagPocketState {
  id: number;
  name: string;
  items: BattleBagItemState[];
}

const DEFAULT_BATTLE_BAG: BattleBagPocketState[] = [
  {
    id: 1,
    name: "ITEMS",
    items: [
...
export function createBattleBagState(): BattleBagPocketState[] {
  return structuredClone(DEFAULT_BATTLE_BAG) as BattleBagPocketState[];
}

export function toBattleBagView(pockets: BattleBagPocketState[]): BattleBagPocketView[] {
...
export function findBattleBagItem(
  pockets: BattleBagPocketState[],
  itemId: string,
): {
  pocket: BattleBagPocketState;
